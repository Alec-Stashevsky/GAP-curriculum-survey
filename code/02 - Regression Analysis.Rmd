...
To assess the affect of the video on respondent's view on including climate change curricula, we run a multivariate linear regression where the independent variable is the change ($\Delta$) between there response before and after watching the accompanying video. We must treat this change as continuous because $\Delta_i = L3_i - L2_i$. Our regression will be of the form $$\Delta_i = D1_i + D4_i + D5_i + D6_i + L1_i + \epsilon_i$$ where D*X* indicates categorical demographic variables and L*X* indicates ordinal Likert scale variables as previously.
